...
public class NumberEngine{
    public static void main(String[] args) throws IOException{
        double[] vals = {1,4,5,9,10,12,10};
        int numSi = 41, numAl = 7, numNa = 7, numO = 96;
        for(int i = 0; i<vals.length; i++){
            vals[i] = (vals[i]*1000)/(numSi*28.08 + numAl*26.98 + numNa*22.99 + numO*16);
        }
        System.out.println(Arrays.toString(vals));
        System.out.println(Arrays.toString(getValues(vals, new double[]{.7,2,3,9,30,40,70})));
    }
    ...
    static void method2(String file) throws IOException{
        Scanner sc = new Scanner(new File("C://users//aul12//Downloads//dlmonte_sims//" + file));
        sc.nextLine();
        int mod = 0;
